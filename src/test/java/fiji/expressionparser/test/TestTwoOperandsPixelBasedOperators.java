...
/**
 * Test cases for parser functions that are part of the
 * {@link ImgLibOperatorSet}. As such, they have an operator-like syntax, such
 * as A + B.
 * 
	...
	public void lowerThan() throws ParseException
	{
		...
		try
		{
			while ( rc.hasNext() )
			{
				rc.fwd();
				ca.setPosition( rc );
				assertEquals( 0.0f, rc.get().getRealFloat(), Float.MIN_VALUE ); // never
																				// true
			}
		}
		catch ( AssertionError ae )
	...
	public void greaterOrEqual() throws ParseException
	{
		...
		try
		{
			while ( rc.hasNext() )
			{
				rc.fwd();
				ca.setPosition( rc );
				assertEquals( 1.0f, rc.get().getRealFloat(), Float.MIN_VALUE ); // always
																				// true
			}
		}
		catch ( AssertionError ae )
